...
import project.data.Actor;
import project.data.Film;
...
public class UserService {
	...
	public Film addFilm(Film f) {
		int id = getNewId();
		int insert_effectue = connect.update("INSERT INTO film VALUES ("+id+",'"+f.getTitre()+"')","film");
		if(insert_effectue == 0) {
	...
	public Actor addActor(Actor a) {
		int id = getNewId();
		int insert_effectue = connect.update("INSERT INTO acteur VALUES ("+id+",'"+a.getNom_acteur()+"')","acteur");
		if(insert_effectue == 0) {
			System.out.println("Problem add film");
			return null;
		}
